...
Add bold, italic, links, headings, code blocks, and more to your messages.
...
Under the hood pyokbot uses `sulguk` to convert your HTML into OK.ru's entity format. The mapping is straightforward — `<b>` becomes `STRONG`, `<i>` becomes `EMPHASIZED`, `<a>` becomes `LINK`, etc.

## Notes

- HTML tags must be properly closed. Unclosed tags may be ignored.
- `parse_mode` defaults to `None` (plain text). You must explicitly set it to `"html"`.
- You can nest tags: `<b><i>bold italic</i></b>` works as expected.
